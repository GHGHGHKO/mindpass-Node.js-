    ...
    if(!(id && nic && name && tel && pw && pwCheck)){      //제목오류
      ...
      if(pw != pwCheck){
        ...
        client.query('insert into mbrdata (id, pw, tel, name, nic ,d_modify, d_regis) values (?, ?, ?, ?, ?, ?, ?)',[id, pw, tel, name, nic, d ,d ],function(err, result){
          if(err){
            client.query('select * from mbrdata where id = ? and nic = ?',[id, nic],function(error, row){
                if(row){
                  res.json({code:500});
                  console.log(err)
                }else{
                  res.json({code:501});
                  console.log('register faild.(dup id or nic)')
                }
            })
          }else{
            client.query('create table '+ id + ' (id int(11) auto_increment, color text, cloth text, cloth_big text, image LONGBLOB not null, PRIMARY KEY(id))',function(err, result){
              if(err){
                res.json({code:500});
                console.log(err)
              }else{
